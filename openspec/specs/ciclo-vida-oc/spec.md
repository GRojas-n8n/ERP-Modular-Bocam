...
El ciclo de vida de una Orden de Compra incluye los estados `PARCIALMENTE_RECIBIDA` y `RECIBIDA` para reflejar el progreso de recepción de materiales.
...
```
BORRADOR → PENDIENTE → APROBADA → EMITIDA → PARCIALMENTE_RECIBIDA → RECIBIDA
                                                                   ↗
                                              EMITIDA ────────────
```
