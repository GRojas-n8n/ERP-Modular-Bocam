...
El middleware compartido `requireProjectAccess()` (`packages/auth-middleware`, usado por todos los microservicios) trata al rol real `'finanzas'` (español) como rol de nivel tenant (acceso a todos los proyectos), igual que `'admin'` y `'superintendent'`. `'procurement'` (Compras) queda explícitamente excluido de este bypass — permanece estrictamente acotado por proyecto (ver `aislamiento-proyecto-por-modulo`).
