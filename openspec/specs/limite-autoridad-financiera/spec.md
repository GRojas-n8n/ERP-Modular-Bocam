# limite-autoridad-financiera Specification

## Purpose

Formaliza en qué mutaciones de Finanzas se ejerce el Límite de Autoridad
Financiera y en cuáles no, y por qué: el límite se valida al ejecutar un
movimiento de dinero, no al planificarlo.

## Requirements

### Requirement: Toda mutación que mueva dinero SHALL validar el Límite de Autoridad Financiera
Toda mutación de Finanzas que descuente saldo de una cuenta bancaria, consuma el anticipo de un proyecto, o transfiera monto entre partidas presupuestales SHALL
...
`POST /pagos` y `POST /pagos/bulk` crean pagos en estado programado sin mover dinero. NO SHALL validar `limite_aprobacion`: el límite se ejerce al ejecutar el
