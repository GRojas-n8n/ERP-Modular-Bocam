...
Cuando GT aprueba un presupuesto (`PATCH /presupuestos/:id/aprobar`), SHALL crear automáticamente un `SaldoPartida` por cada `Concepto` del presupuesto.
...
Nómina autorizada (MANO_OBRA) → monto_comprometido += total_neto
Nómina pagada             → monto_ejercido       += total_neto
                             monto_comprometido   -= total_neto
