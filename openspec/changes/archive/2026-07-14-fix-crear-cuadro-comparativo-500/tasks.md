...
- [x] 4.3 Verificación manual en navegador contra el caso real: crear el cuadro comparativo
      ...
      una Requisicion real con `especificacion_marca_modelo` de exactamente
      109 caracteres + Solicitud de Cotización con 1 proveedor `RESPONDIO`
      (sembradas vía script Prisma de un solo uso, script y test borrados
      tras usarlos). Clic real en "Crear Cuadro Comparativo" →
      `POST /comparativas` responde 201 (no 500), el cuadro abre
      normalmente ("Tabla de Cotizaciones" visible) sin ningún toast de
      error.
