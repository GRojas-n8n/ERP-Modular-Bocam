...
- [x] 4.5 Verificación manual en navegador: abrir el panel de una
      ...
      change).
      Verificado con Playwright real (`comprador@alfa.bocam.com`), con
      `apps/almacen` levantado, contra un `ItemInventario` real (50 en
      stock) para un insumo real de catálogo GT + una Requisición real con
      ese insumo (cantidad 20) — sembrados vía scripts Prisma de un solo
      uso, borrados tras usarlos. Al abrir "Enviar Solicitud de
      Cotización": aparece "⚠ Ya hay stock disponible en Almacén" con el
      detalle correcto ("se piden 20, hay 50 en almacén"); el botón de
      envío normal está oculto hasta confirmar; clic en "Entiendo, enviar
      de todos modos" revela el flujo de envío normal; seleccionar 1
      proveedor y enviar responde 200 (`POST .../solicitud-cotizacion`
      exitoso, mismo endpoint que antes de este change).
