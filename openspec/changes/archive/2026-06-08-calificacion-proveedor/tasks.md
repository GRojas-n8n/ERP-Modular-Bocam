...
- [x] 1.1 Agregar modelo `CalificacionProveedor` en `apps/compras/prisma/schema.prisma`:
...
- [x] 1.2 Agregar relación `calificaciones CalificacionProveedor[]` en modelo `Proveedor`
- [x] 1.3 Ejecutar `npx prisma generate` en `apps/compras` para actualizar el cliente

## 2. Migración SQL manual

- [x] 2.1 Crear directorio `apps/compras/prisma/migrations/20260608150000_add_calificaciones_proveedor/`
- [x] 2.2 Escribir `migration.sql`:
  ```sql
...
- [x] 3.1 Implementar `POST /api/v1/compras/proveedores/:id/calificaciones` — `requireRoles('procurement', 'admin')`:
...
- [x] 4.1 Implementar `GET /api/v1/compras/proveedores/:id/calificaciones` — `requireRoles('procurement', 'admin', 'finance', 'gerencia_tecnica', 'superintendent')`:
  - Verificar que el proveedor existe (`tenant_id` scope)
...
- [x] 5.1 Implementar `DELETE /api/v1/compras/proveedores/:id/calificaciones/:cid` — `requireRoles('admin')`:
...
- [x] 6.1 Agregar estado en `ComprasView`:
...
- [x] 6.2 Implementar `fetchCalHistorial(proveedorId)` que llama `GET /api/v1/compras/proveedores/:id/calificaciones` y popula `calHistorial` y `calPromedios`
- [x] 6.3 Agregar botón **★ Calificar** en cada fila de la tabla de proveedores (junto a 📎 Docs y Editar), visible solo para `isProcurement`. Al hacer clic: `setCalHistorialId(id)` + `fetchCalHistorial(id)`

## 7. Frontend — Columna de score enriquecida en tabla

- [x] 7.1 Actualizar la columna de score en la tabla de proveedores:
...
- [x] 7.2 En el formulario de edición de proveedor (`SlidePanel`), ocultar el campo `calificacion_desempeno` o mostrarlo como texto read-only con label "Score calculado automáticamente"

## 8. Frontend — SideSheet de historial y calificación

- [x] 8.1 Agregar `SlidePanel` que se abre cuando `calHistorialId !== null` con `accentColor="amber"` y título "Historial de Calificaciones":
...
- [x] 8.2 Sección de formulario (solo `isProcurement`) en el mismo SideSheet:
...
- [x] 9.1 Copiar y ejecutar `migration.sql` en VPS: `prisma db execute --file migration.sql`
- [x] 9.2 Registrar migración: `prisma migrate resolve --applied 20260608150000_add_calificaciones_proveedor`
- [x] 9.3 Commit + push al repo, `git pull` en VPS, rebuild: todos los contenedores `Healthy`
- [x] 9.4 Verificar: calificación registrada, score aparece en tabla ✓
- [x] 9.5 Verificar: promedio recalcula correctamente ✓
- [x] 9.6 Verificar: historial muestra entradas con proyecto y calificador ✓
