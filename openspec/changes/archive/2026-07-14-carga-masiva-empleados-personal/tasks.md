...
- [x] 4.2 Verificación manual: confirmar que un usuario sin rol
      `personal_rh`/`admin` no ve el botón "Importar CSV/Excel" y que,
      si llama al endpoint directamente, recibe 403.
      Automatizado en `empleados-importar-lote.e2e.spec.ts` (segundo
      test): login como `comprador@alfa.bocam.com` (rol `procurement`)
      — sin acceso al módulo Recursos Humanos, botón inexistente en el
      DOM. Llamada directa a
      `POST /api/v1/personal/empleados/importar-lote` con su JWT real →
