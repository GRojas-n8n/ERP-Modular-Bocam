...
- [x] 8.1 Verificación manual en navegador contra un caso real: la requisición de la prueba
      ...
      No se tuvo acceso a esa requisición real de producción en este
      entorno. Verificado en su lugar con Playwright real
      (`comprador@alfa.bocam.com`) contra una Requisicion real con un
      ítem de texto libre (sin `insumo_id`, script Prisma de un solo uso,
      borrado tras usarlo) + Solicitud con 1 proveedor `RESPONDIO`. Clic
      real en "Crear Cuadro Comparativo" → `POST /comparativas` responde
      201 (la línea sin `insumo_id` sí se crea, ya no se salta). Precio
      capturado manualmente (2750) → "Enviar a Evaluación Técnica →" →
      `PUT .../cotizaciones` responde 200 (el precio sin `insumo_id` ya no
      se descarta). Confirmado contra el backend (`GET /comparativas`,
      equivalente a recargar la página): el precio persistió con
      `insumo_id: null` y `detalle_req_id` poblado, como se diseñó.
...
- [x] 9.1 Sincronizar `openspec/specs/cotizacion-compras-ux/spec.md` con la spec delta de
