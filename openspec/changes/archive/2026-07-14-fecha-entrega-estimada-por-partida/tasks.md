...
- [x] 5.1 Con el usuario `procuracion@bocam.com.mx`: capturar precios y
      ...
      **PENDIENTE — requiere backend completo levantado con datos reales;
      ...
      Verificado localmente con Playwright real (`comprador@alfa.bocam.com`,
      no había usuario `procurement` en el seed usado para esta
      verificación) contra una Requisicion + Solicitud reales con 2
      proveedores `RESPONDIO` (script Prisma de un solo uso, borrado tras
      usarlo). Capturados precio + fecha de entrega para ambos proveedores
      en la Tabla de Cotizaciones, "Enviar a Evaluación Técnica →" →
      `PUT .../cotizaciones` responde 200. Confirmado contra el backend
      (`GET /comparativas`, equivalente a una recarga real): ambas fechas
      (2026-08-01 y 2026-08-05) persistieron correctamente, cada una en el
      `ComparativaDetalle` de su proveedor correspondiente, no mezcladas.
