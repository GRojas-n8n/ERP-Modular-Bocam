...
- [x] 3.2 Verificación manual en navegador contra un caso real: crear un Cuadro Comparativo
      ...
      Verificado con Playwright real (`comprador@alfa.bocam.com`) contra
      una Requisicion real con ítem de catálogo (`especificacion_marca_modelo`/
      `especificacion_detalle` capturados, sin `EspecificacionDetalleReq`)
      + Solicitud con 1 proveedor `RESPONDIO` (script Prisma de un solo
      uso, borrado tras usarlo). Clic real en "Crear Cuadro Comparativo" →
      el panel "Detalles técnicos" ya muestra "Mirage, United Appliances
      mod. X200" y la especificación completa, sin que Compras tuviera que
      capturarlas.
...
- [x] 4.1 Sincronizar `openspec/specs/cotizacion-compras-ux/spec.md` con la spec delta de
