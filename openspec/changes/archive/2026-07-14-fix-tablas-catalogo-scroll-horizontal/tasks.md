...
- [x] 7.2 Verificación manual en navegador: Catálogo de Obra y al menos 2 de las
      ...
      reales (catálogo GT ya sembrado) con viewport de 800px de ancho:
      `table-scroll-shadow-right` (testid del componente
      `TableScrollShadow`) visible en Catálogo de Obra, Control de Costos
      (WBS) y Compras → Proveedores — las 3 tablas señaladas en el
      inventario del bug. A 1024px, Proveedores ya no desborda (tabla más
      angosta) y correctamente no muestra la sombra, confirmando que la
      señal es dinámica y no un elemento siempre visible.
