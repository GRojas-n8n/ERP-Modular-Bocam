...
- [x] 4.3 Verificación manual en navegador: subir un Excel con encabezados en español
      ...
      Verificado con Playwright real (`admin@alfa.bocam.com`) subiendo un
      CSV con encabezados "RFC,Razón Social,Email,Teléfono" (con tildes y
      espacios, no snake_case) al importador de Clientes — vista previa:
      1 listo para importar, 0 con error. Antes del fix, ninguno de estos
      encabezados emparejaba con una columna. Test descartado tras usarlo
      (no se llegó a confirmar la importación real, solo la vista previa).

## 5. Cierre

- [x] 5.1 Sincronizar `openspec/specs/carga-masiva-archivos/spec.md` (nuevo) al archivar.
