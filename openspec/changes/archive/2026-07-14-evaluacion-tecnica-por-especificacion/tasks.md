...
- [x] 8.3 Verificación manual en navegador: evaluar un renglón con 2+
      ...
      Verificado contra un CuadroComparativo real en EN_EVALUACION_TECNICA
      con 1 renglón de texto libre y 2 especificaciones reales, 2
      proveedores (script Prisma de un solo uso, borrado tras usarlo —
      importante: además del `CuadroComparativo`/`ComparativaDetalle` hay
      que sembrar también `ComparativaLinea` con el mismo `detalle_req_id`;
      sin esa fila la sub-vista por característica no aparece, cae al
      renglón simple). Con Playwright real (`residente@alfa.bocam.com`):
      las 2 sub-filas por característica se ven sin clics adicionales;
      marcar "?" en una característica con pregunta y "Guardar" hace
      aparecer el footer "Enviar dudas y crear revisión"; al confirmarlo,
      se crea exactamente 1 revisión nueva (confirmado contra `GET
      /comparativas`). Cierre del ciclo confirmado contra el backend real
      (`comprador@alfa.bocam.com`, vía API): `GET
      /comparativas/:id` de la revisión nueva devuelve la
      `EvaluacionEspecificacion` clonada con `pregunta_residente` heredada
      y reseteada a `PENDIENTE`; `PUT .../responder-preguntas` guarda
      `respuesta_compras` correctamente. La verificación de este último
      tramo (que Compras vea el cuadro de solo-lectura correcto y responda
      desde la UI) se completó contra la API en vez de la UI por
      inestabilidad del navegador automatizado con múltiples
      logins/contextos en esta sesión — el contrato backend↔frontend que
      consume la UI (mismo endpoint, mismo shape de datos) quedó
      verificado end-to-end igualmente.
