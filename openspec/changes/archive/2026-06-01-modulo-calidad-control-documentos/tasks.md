...
- [x] 1.1 Crear directorio `apps/calidad/` con estructura estándar:
  ```
  apps/calidad/
...
- [x] 1.2 Crear `apps/calidad/package.json` siguiendo el patrón de `apps/compras/package.json`:
...
- [x] 1.3 Crear `apps/calidad/tsconfig.json` copiando el de otro módulo (target ES2022, module NodeNext, strict: true, verbatimModuleSyntax: true).

- [x] 1.4 Crear `apps/calidad/.env` con variables:
  ```
...
- [x] 2.1 Crear `apps/calidad/prisma/schema.prisma` con los modelos `Documento` y `VersionDocumento` según el design.md. Incluir `generator client`, `datasource db` y RLS setup.

- [x] 2.2 Ejecutar `npx prisma migrate dev --name init-calidad` en `apps/calidad/`.

- [x] 2.3 Verificar que el SQL generado incluye las tablas `documentos` y `versiones_documento` con todos los índices y la restricción `@@unique([tenant_id, codigo])`.

- [x] 2.4 Ejecutar `npx prisma generate` y verificar que `src/generated/prisma/` contiene los tipos TypeScript.

- [x] 2.5 Aplicar políticas RLS en PostgreSQL para ambas tablas:
  ```sql
...
- [x] 3.1 Crear `apps/calidad/src/db.ts` copiando el patrón de otro módulo: exportar `createTenantContext`, `runAsSystem`, `disconnectDb`. Usar `PrismaClient` del `generated/prisma/`.

- [x] 3.2 Crear `apps/calidad/src/types.ts` con:
...
- [x] 4.1 Crear `apps/calidad/src/main.ts` con:
...
- [x] 4.2 Implementar `GET /api/v1/calidad/dashboard`:
...
- [x] 4.3 Implementar `GET /api/v1/calidad/documentos`:
  - `requireRoles('calidad', 'admin', 'superintendent')`
...
- [x] 4.4 Implementar `POST /api/v1/calidad/documentos`:
...
- [x] 4.5 Implementar `GET /api/v1/calidad/documentos/:id`:
  - `requireRoles('calidad', 'admin', 'superintendent')`
  - Include `versiones` ordenadas por `created_at DESC`
  - `404` si no existe en el tenant

- [x] 4.6 Implementar `PATCH /api/v1/calidad/documentos/:id`:
  - `requireRoles('calidad', 'admin')`
  - Solo actualizar campos permitidos: `titulo`, `descripcion`, `responsable_id`, `proyecto_id`
  - `404` si no existe

- [x] 4.7 Implementar `DELETE /api/v1/calidad/documentos/:id`:
  - `requireRoles('calidad', 'admin')`
...
- [x] 4.8 Configurar middleware `multer` para upload:
  ```typescript
...
- [x] 4.9 Implementar `POST /api/v1/calidad/documentos/:id/versiones`:
  - `requireRoles('calidad', 'admin')`
...
- [x] 4.10 Implementar `PATCH /api/v1/calidad/documentos/:id/versiones/:vid/estado`:
  - `requireRoles('calidad', 'admin')`
...
- [x] 4.11 Implementar `GET /api/v1/calidad/documentos/:id/versiones/:vid/archivo`:
  - `requireRoles('calidad', 'admin', 'superintendent')`
...
- [x] 5.1 Crear `apps/app-shell/src/views/CalidadView.tsx`:
...
- [x] 5.2 Implementar sección **Dashboard / KPIs** (visible siempre arriba):
...
- [x] 5.3 Implementar sección **Lista de Documentos**:
...
- [x] 5.4 Implementar **SlidePanel "Nuevo Documento"**:
...
- [x] 5.5 Implementar **vista de Detalle de Documento** (inline o SlidePanel ancho):
...
- [x] 5.6 Implementar **SlidePanel "Nueva Versión"**:
...
- [x] 5.7 Implementar **descarga de archivo**:
...
- [x] 5.8 Implementar confirmación para acción "Marcar como Obsoleto":
...
- [x] 6.1 En `Layout.tsx`, agregar en `ALL_NAV_ITEMS`:
  ```typescript
  {
    name: 'Calidad',
...
- [x] 6.2 En `App.tsx`, agregar en `renderView()`:
  ```typescript
  case 'calidad':
...
- [x] 7.1 Crear `docker/Dockerfile.calidad` siguiendo el patrón de `docker/Dockerfile.compras` (o similar):
...
- [x] 7.2 Agregar servicio `calidad` en `docker-compose.vps.yml`:
  ```yaml
  calidad:
    profiles: ["core", "full"]
    build:
...
- [x] 7.3 Agregar volumen `vps_calidad_uploads` en la sección `volumes:` de `docker-compose.vps.yml`.

- [x] 7.4 Agregar variable de entorno `CALIDAD_DATABASE_URL` al `.env` del VPS.

## 8. Nginx — Proxy hacia calidad

- [x] 8.1 En `docker/nginx.qnap.conf`, agregar bloque de proxy para calidad:
  ```nginx
...
- [x] 9.1 Verificar que el rol `calidad` esté en el catálogo de roles del módulo `auth` (`apps/auth/src/main.ts` o donde se define el ROLES_CATALOG). Si no está, agregarlo.

- [x] 9.2 En `AdminView.tsx` del frontend, verificar que `calidad` aparezca en el selector de roles al crear/editar usuarios. Está en el array `ROLES_CATALOG` del `AdminView`. Si no está, agregarlo:
  ```typescript
...
- [x] 10.1 Actualizar la tabla de puertos (§16): agregar `calidad | 3009 | módulo SGC ISO 9001`
- [x] 10.2 Actualizar la tabla de roles (§11): agregar `calidad | Gestión de Calidad — documentos, versiones, SGC ISO 9001`

## 11. Deploy a VPS

- [x] 11.1 Aplicar migración en VPS:
  ```bash
...
- [x] 11.2 Build y deploy del módulo calidad:
  ```bash
...
- [x] 11.3 Rebuild y deploy del app-shell (cambios en Layout, App y CalidadView):
  ```bash
