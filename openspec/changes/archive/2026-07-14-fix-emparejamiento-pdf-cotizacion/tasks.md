...
- [x] 4.3 Verificación manual en navegador (Playwright o manual) contra un caso real:
      ...
      No se tuvo acceso a esa requisición real de producción ni al
      microservicio `asistente` (requiere `ANTHROPIC_API_KEY`) en este
      entorno. Verificado en su lugar con Playwright real contra un
      CuadroComparativo real (BORRADOR, script Prisma de un solo uso,
      borrado tras usarlo) con una línea "Mini Split Inverter de 1
      Tonelada (12,000 BTU) a 220V" (mismo texto del caso real) — la
      llamada de red a `apps/asistente` se interceptó (`page.route`) para
      devolver el renglón real del bug reportado ("Minisplit Inverter 1
      Ton 220V", sin compartir los primeros 10 caracteres con la línea),
      ejerciendo el `emparejarRenglonesConLineas` real del bundle. Subida
      real de un PDF dummy vía el input de archivo real → vista previa
      correcta → "Aplicar al cuadro" → `PUT
      .../proveedores/:id/cotizacion-pdf` responde 200, precio 4500
      aplicado a la línea correcta, toast "Cotización aplicada" (no el de
      match parcial). Confirma el fix end-to-end en el navegador real sin
      depender de la infraestructura de IA.
