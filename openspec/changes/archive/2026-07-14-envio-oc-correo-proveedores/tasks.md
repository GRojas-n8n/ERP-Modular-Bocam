...
- [x] 6.2 Verificación manual en navegador: generar una OC real (o usar una
      ...
      OrdenCompra real sembrada vía script Prisma (proveedor con
      `email_contacto` real, script borrado tras usarlo) — con `apps/reportes`
      levantado (`REPORTES_SERVICE_URL` local) y SMTP real de prueba
      (cuenta Ethereal desechable creada solo para esta verificación,
      credenciales retiradas de `.env` al terminar). Resultado:
      `{enviadas:1, fallidas:0}`, `enviada_proveedor_at`/`enviada_proveedor_email`
      persistidos, indicador "Enviada el" visible en el listado, sin el
      error `SMTP no configurado` que bloqueaba esta verificación antes —
      el handshake SMTP real tomó ~4.8s (consistente con un envío de red
      real, no un stub). No se confirmó visualmente el adjunto PDF dentro
      del inbox de Ethereal (fuera del alcance de esta sesión), pero el PDF
      se generó exitosamente vía `apps/reportes` antes del envío (sin esa
      generación, el endpoint habría reportado la OC como fallida).
