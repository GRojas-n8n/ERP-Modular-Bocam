...
Almacén no consulta a Compras ni a Gerencia Técnica al procesar. El snapshot lo resuelve Compras **en la petición de recepción**, con el token del usuario, antes de abrir la transacción, y lo congela en el registro del outbox: para insumos de catálogo consulta a Gerencia Técnica (`clave`, `descripcion`, `unidad` y la categoría a partir del tipo de insumo); para texto libre copia `descripcion_libre` y `unidad_libre`. El despachador no resuelve nada: corre sin usuario y no debe fabricar ni almacenar tokens. Si Gerencia Técnica no responde, la recepción no se bloquea: el evento sale sin snapshot para esos ítems y no se inventan datos. Almacén crea un ítem de inventario solo si el evento trae snapshot completo; si falta y el insumo no existe en su inventario, lo envía a la cola de mensajes fallidos (`SNAPSHOT_INCOMPLETO`), y al reprocesarlo tras crear el ítem se aplica. El snapshot no cambia en los reintentos de publicación.
...
- **Despachador.** Un proceso interno de Compras toma filas `PENDIENTE` cuyo `proximo_intento_en` ya venció, con bloqueo `FOR UPDATE SKIP LOCKED` para que varias instancias no dupliquen trabajo, publica con confirmación y solo entonces marca `PUBLICADO`. Un fallo incrementa `intentos` y reprograma con espera exponencial; superado un umbral pasa a `ERROR` y queda visible.
