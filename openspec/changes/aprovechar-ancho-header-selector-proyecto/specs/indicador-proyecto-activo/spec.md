...
indicador colapsado. El indicador SHALL poder ocupar el espacio horizontal disponible en el
header (no un ancho fijo arbitrario que lo limite antes de agotar el espacio real disponible).
...
#### Scenario: El indicador aprovecha el espacio horizontal disponible del header
- **WHEN** el header no tiene otros elementos compitiendo por espacio del lado del indicador
  (p. ej. tras eliminar el badge decorativo "Sistema sincronizado")
- **THEN** el indicador de proyecto crece para ocupar el espacio disponible en vez de quedar
  acotado a un ancho fijo pequeño, mostrando más del nombre del proyecto antes de truncarlo
