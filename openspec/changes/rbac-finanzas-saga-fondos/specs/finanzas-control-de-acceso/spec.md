...
Los endpoints `POST /api/v1/finanzas/comprometer-fondos` y
`POST /api/v1/finanzas/liberar-fondos` SHALL rechazar con 403 toda petición cuyo
JWT no incluya al menos uno de los roles `finanzas`, `admin`, `superintendent` o
`procurement`, antes de ejecutar cualquier lectura o escritura contra la base de
datos y antes de publicar cualquier evento de dominio.
