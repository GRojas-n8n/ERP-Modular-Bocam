...
const almacenDatabaseUrl = process.env.ALMACEN_DATABASE_URL || process.env.DATABASE_URL;
const basePrisma = new PrismaClient({
  ...(almacenDatabaseUrl ? { datasources: { db: { url: almacenDatabaseUrl } } } : {}),
...
export async function createTenantContext<T>(
  ...
  return await basePrisma.$transaction(async (tx) => {
    ...
    await tx.$executeRaw`SELECT set_config('app.current_proyecto_id', ${context.proyectoId}, true)`;
