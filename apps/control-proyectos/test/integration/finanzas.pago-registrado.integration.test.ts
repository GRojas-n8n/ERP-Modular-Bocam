...
import { PrismaClient as ControlProyectosPrismaClient } from '../../src/generated/prisma';
...
const cpDbUrl = process.env.DATABASE_URL || 'postgresql://postgres:bocam_dev_password@localhost:5432/bocam_erp?schema=control_proyectos';
const finanzasDbUrl = cpDbUrl.includes('schema=control_proyectos')
  ? cpDbUrl.replace('schema=control_proyectos', 'schema=finanzas')
  : 'postgresql://postgres:bocam_dev_password@localhost:5432/bocam_erp?schema=finanzas';

const cpPrisma = new ControlProyectosPrismaClient({ datasources: { db: { url: cpDbUrl } } });
const finanzasPrisma = new FinanzasPrismaClient({ datasources: { db: { url: finanzasDbUrl } } });
...
async function seedScenario() {
  ...
  return { tenantId, proyectoId, userId, estimacionId: estimacion.id_estimacion, pagoId: pago.id_pago };
}

async function main() {
  process.env.RABBITMQ_URL = rabbitUrl;
  process.env.CONTROL_PROYECTOS_DATABASE_URL = cpDbUrl;
  ...
  const consumer = createEventBus(`control-proyectos-it-${randomUUID()}`);
  ...
  try {
    ...
    const response = await fetch(`${finanzasBaseUrl}/api/v1/finanzas/pagos/${seeded.pagoId}/pagar`, {
      ...
      body: JSON.stringify({ referencia_bancaria: 'TRX-123456', metodo_pago: 'TRANSFERENCIA', banco: 'BBVA' }),
    });

    assert.equal(response.status, 200);

    await waitFor(async () => {
      const estimacion = await createTenantContext({ tenantId: seeded.tenantId, proyectoId: seeded.proyectoId, userId: seeded.userId }, async (tx) =>
        tx.estimacion.findUniqueOrThrow({ where: { id_estimacion: seeded.estimacionId } })
      );
      assert.equal(estimacion.estado, 'FACTURADA');
      assert.ok(estimacion.notas?.includes(seeded.pagoId));
    });

    const afterFirst = await createTenantContext({ tenantId: seeded.tenantId, proyectoId: seeded.proyectoId, userId: seeded.userId }, async (tx) =>
      tx.estimacion.findUniqueOrThrow({ where: { id_estimacion: seeded.estimacionId } })
    );
    ...
    console.log('ok - ciclo completo finanzas.pago_registrado -> control-proyectos cierra estimacion pagada');
  } finally {
...
void main().catch((error) => {
  console.error('not ok - integracion finanzas.pago_registrado -> control-proyectos');
  console.error(error);
  process.exitCode = 1;
});
