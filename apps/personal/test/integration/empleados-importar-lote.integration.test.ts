/**
 * ---------------------------------------------------------------------------
 * Tests de Integración: POST /empleados/importar-lote (Personal)
 * Spec:  openspec/changes/carga-masiva-empleados-personal/specs/carga-masiva-empleados/
 * Tarea: 1.10 del tasks.md
 *
 * Runner: node -r ts-node/register/transpile-only <este-archivo>
 * Requiere: PostgreSQL corriendo (DATABASE_URL en .env)
 * ---------------------------------------------------------------------------
 */

process.env.JWT_SECRET = process.env.JWT_SECRET || 'bocam-e2e-secret';

import assert from 'node:assert/strict';
import { randomUUID } from 'node:crypto';
import type { Server } from 'node:http';
import { PrismaClient } from '../../src/generated/prisma';
import { signTenantToken, startHttpApp, stopHttpApp } from '../../../../test-support/e2e';

const personalDbUrl =
  process.env.PERSONAL_DATABASE_URL ||
  process.env.DATABASE_URL ||
  'postgresql://postgres:bocam_dev_password@localhost:5432/bocam_erp?schema=personal';

const prisma = new PrismaClient({ datasources: { db: { url: personalDbUrl } } });

let personalServer: Server | undefined;
let personalBaseUrl = '';

async function setup() {
  const personalModule = await import('../../src/main');
  const started = await startHttpApp(personalModule.app);
  personalServer = started.server;
  personalBaseUrl = started.baseUrl;
}

async function teardown() {
  await stopHttpApp(personalServer);
  await prisma.$disconnect();
}

async function cleanupTenant(tenantId: string) {
  await prisma.empleado.deleteMany({ where: { tenant_id: tenantId } });
}

async function post(path: string, token: string, body: object) {
  return fetch(`${personalBaseUrl}${path}`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });
}

// ── Test: lote 100% válido, numero_empleado correlativo sin huecos ─────────

async function testLoteTodosValidosNumeroCorrelativo() {
  const tenantId = randomUUID();
  const proyectoId = randomUUID();
  const userId = randomUUID();
  const token = signTenantToken({ userId, tenantId, proyectoId, roles: ['personal_rh'] });

  try {
    const sufijo = Date.now().toString().slice(-6);
    const r = await post('/api/v1/personal/empleados/importar-lote', token, {
      registros: [
        { nombre: 'Juan', apellido_paterno: 'Pérez', rfc: `PEJ${sufijo}A`, puesto: 'Fierrero', salario_diario: 350 },
        { nombre: 'Ana', apellido_paterno: 'García', rfc: `GAA${sufijo}B`, puesto: 'Operador', salario_diario: 400 },
      ],
    });
    assert.equal(r.status, 200, 'lote 100% válido debe responder 200');
    const body = (await r.json()) as any;
    assert.equal(body.data.creados, 2, 'ambos registros deben crearse');
    assert.equal(body.data.errores.length, 0, 'no debe haber errores');

    const numeros: string[] = body.data.empleados.map((e: any) => e.numero_empleado).sort();
    const numsInt = numeros.map(n => parseInt(n.replace('EMP-', '')));
    assert.equal(numsInt[1] - numsInt[0], 1, 'los numero_empleado deben ser correlativos sin huecos');

    console.log('ok - lote 100% válido crea todos los registros con numero_empleado correlativo');
  } finally {
    await cleanupTenant(tenantId);
  }
}

// ── Test: lote mixto (sin campos obligatorios, salario_diario no numérico) ─

async function testLoteMixto() {
  const tenantId = randomUUID();
  const proyectoId = randomUUID();
  const userId = randomUUID();
  const token = signTenantToken({ userId, tenantId, proyectoId, roles: ['personal_rh'] });

  try {
    const sufijo = Date.now().toString().slice(-6);
    const r = await post('/api/v1/personal/empleados/importar-lote', token, {
      registros: [
        { nombre: 'Valido', apellido_paterno: 'Lote', rfc: `VAL${sufijo}A`, puesto: 'Técnico', salario_diario: 300 },
        { nombre: 'Sin Puesto', apellido_paterno: 'Lote', rfc: `SIN${sufijo}B`, puesto: '', salario_diario: 300 },
        { nombre: 'Salario Malo', apellido_paterno: 'Lote', rfc: `SAL${sufijo}C`, puesto: 'Técnico', salario_diario: 'no-es-numero' },
      ],
    });
    assert.equal(r.status, 200, 'lote mixto debe responder 200 (no aborta por errores parciales)');
    const body = (await r.json()) as any;
    assert.equal(body.data.creados, 1, 'solo el registro válido debe crearse');
    assert.equal(body.data.errores.length, 2, 'los 2 registros inválidos deben reportarse');
    assert.equal(body.data.errores[0].fila, 2);
    assert.equal(body.data.errores[1].fila, 3);
    assert.match(body.data.errores[1].motivo, /numérico/);

    console.log('ok - lote mixto crea válidos y reporta inválidos por fila sin abortar (incluye salario_diario no numérico)');
  } finally {
    await cleanupTenant(tenantId);
  }
}

// ── Test: RFC duplicado dentro del mismo archivo ────────────────────────────

async function testRfcDuplicadoDentroDelArchivo() {
  const tenantId = randomUUID();
  const proyectoId = randomUUID();
  const userId = randomUUID();
  const token = signTenantToken({ userId, tenantId, proyectoId, roles: ['personal_rh'] });

  try {
    const sufijo = Date.now().toString().slice(-6);
    const rfcRepetido = `DUP${sufijo}`;
    const r = await post('/api/v1/personal/empleados/importar-lote', token, {
      registros: [
        { nombre: 'Primera', apellido_paterno: 'Fila', rfc: rfcRepetido, puesto: 'Obrero', salario_diario: 280 },
        { nombre: 'Segunda', apellido_paterno: 'Fila', rfc: rfcRepetido, puesto: 'Obrero', salario_diario: 280 },
      ],
    });
    assert.equal(r.status, 200);
    const body = (await r.json()) as any;
    assert.equal(body.data.creados, 0, 'ninguna de las dos filas con RFC duplicado debe crearse');
    assert.equal(body.data.errores.length, 2, 'ambas filas deben reportarse como error');
    assert.match(body.data.errores[0].motivo, /RFC duplicado/);

    const enBd = await prisma.empleado.findFirst({ where: { tenant_id: tenantId, rfc: rfcRepetido } });
    assert.equal(enBd, null, 'no debe haber quedado ningún registro con ese RFC');

    console.log('ok - RFC duplicado dentro del mismo archivo se rechaza en ambas filas');
  } finally {
    await cleanupTenant(tenantId);
  }
}

// ── Test: rol sin personal_rh/admin recibe 403 ──────────────────────────────

async function testRolSinPermisoEs403() {
  const tenantId = randomUUID();
  const proyectoId = randomUUID();
  const userId = randomUUID();
  const token = signTenantToken({ userId, tenantId, proyectoId, roles: ['residencia'] });

  try {
    const r = await post('/api/v1/personal/empleados/importar-lote', token, {
      registros: [{ nombre: 'No', apellido_paterno: 'Debe Crearse', rfc: 'RNOPERM', puesto: 'X', salario_diario: 200 }],
    });
    assert.equal(r.status, 403, 'rol sin personal_rh/admin debe recibir 403');

    const enBd = await prisma.empleado.findFirst({ where: { tenant_id: tenantId } });
    assert.equal(enBd, null, 'no debe haberse creado ningún registro');

    console.log('ok - rol sin personal_rh/admin (residencia) recibe 403 y no crea nada');
  } finally {
    await cleanupTenant(tenantId);
  }
}

async function main() {
  await setup();
  try {
    await testLoteTodosValidosNumeroCorrelativo();
    await testLoteMixto();
    await testRfcDuplicadoDentroDelArchivo();
    await testRolSinPermisoEs403();
  } finally {
    await teardown();
  }
}

void main().catch((error) => {
  console.error('not ok - empleados-importar-lote integration tests');
  console.error(error);
  process.exitCode = 1;
});
