...
async function main() {
  ...
  const event = {
    ...
    payload: {
      ...
      codigo_centro_costos: 'HCO2026012001',
      empresa_grupo: 'HCO',
      anio_centro_costos: 2026,
      cliente_id: randomUUID(),
      es_especial: false,
      estatus: 'ABIERTO',
      nombre_oficial: 'Proyecto Prueba Evento Control Obra',
  ...
  const linea = capturedLogs.find((l) => l.includes('"action":"control_obra.event.centro_costos_creado.registrado"'));
  ...
  assert.ok(linea!.includes('"codigo_centro_costos":"HCO2026012001"'));

  console.log = originalLog;
  console.log('ok - control-obra registra auth.centro_costos_creado sin lanzar excepción');
}

void main().catch((error) => {
  console.log = originalLog;
  console.error('not ok - integración evento-centro-costos-creado (control-obra)');
  console.error(error);
  process.exitCode = 1;
});
