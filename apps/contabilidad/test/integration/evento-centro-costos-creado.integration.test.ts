...
async function main() {
  ...
  const event = {
    ...
    payload: {
      ...
      codigo_centro_costos: 'HCO2026010001',
      empresa_grupo: 'HCO',
      anio_centro_costos: 2026,
      cliente_id: randomUUID(),
      es_especial: false,
      estatus: 'ABIERTO',
      nombre_oficial: 'Proyecto Prueba Evento Contabilidad',
  ...
  const linea = capturedLogs.find((l) => l.includes('"action":"contabilidad.event.centro_costos_creado.registrado"'));
  ...
  assert.ok(linea!.includes('"codigo_centro_costos":"HCO2026010001"'));

  console.log = originalLog;
  console.log('ok - contabilidad registra auth.centro_costos_creado sin lanzar excepción');
}

void main().catch((error) => {
  console.log = originalLog;
  console.error('not ok - integración evento-centro-costos-creado (contabilidad)');
  console.error(error);
  process.exitCode = 1;
});
