...
async function main() {
  ...
  try {
    ...
    await publisher.publish({
      ...
      payload: {
        oc_id: ocId,
        codigo: 'OC-CONT-CANCEL-001',
        total: 155000,
        proveedor_id: 'proveedor-reversa-demo',
    ...
    await waitFor(async () => {
      ...
      assert.equal(pasivo.estatus, 'REVERTIDO');
      assert.equal(reversa.tipo_poliza, 'REVERSION_PASIVO_PROYECTADO');
      assert.equal(reversa.estatus, 'REVERTIDO');
      assert.ok(reversa.folio_poliza.startsWith('POL-REV-'));
    });

    assert.equal(receivedGeneratedEvents.length, 2);
    ...
    const totalReversas = await contabilidadPrisma.asientoContable.count({
      where: {
        tenant_id: tenantId,
        external_event_key: `compras.oc_cancelada:${ocId}`,
      },
    ...
    assert.equal(totalReversas, 1);
    assert.equal(receivedGeneratedEvents.length, 2);
    console.log('ok - integracion real compras.oc_cancelada -> contabilidad revierte pasivo proyectado');
  } finally {
...
void main().catch((error) => {
  console.error('not ok - integracion compras.oc_cancelada -> contabilidad');
  console.error(error);
  process.exitCode = 1;
});
